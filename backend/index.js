...
if (process.env.NODE_ENV === 'production') {
  ...
  if (fs.existsSync(indexHtml)) {
    ...
    console.warn(`Frontend dist not found at ${indexHtml}`);
  }
}
...
const scheduleRoundEnd = (room) => {
  ...
  room.timer = setTimeout(async () => {
    ...
    io.to(room.id).emit('round_end', {
      ...
      scores: room.leaderboard,
      nextDrawer:
        room.players[(room.turnIndex + 1) % room.players.length]?.name
    });
...
function handleDraw(socket, event, payload = {}) {
  ...
  if (event === 'draw_move' || event === 'draw_end') {
    room.strokes.at(-1)?.push(data);
  }
